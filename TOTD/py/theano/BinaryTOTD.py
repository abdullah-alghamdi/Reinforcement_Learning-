...
class BinaryTOTD:
    ...
    def __init__(self, alpha, lambda_, gamma, theta, phi):
        ...
        self.phi_prime = T.ivector('phi_prime')
        self.alpha = T.fvector('alpha')
        self.lambda_ = T.fvector('lambda')
        self.gamma = T.fvector('gamma')

        #Shared variables
        self.phi = theano.shared(value = np.copy(phi).astype(np.int32), name='phi', borrow=True)
        ...
        V = T.sum(T.take(self.theta, self.phi, axis=1), axis = 1)
        V_prime = T.sum(T.take(self.theta, self.phi_prime, axis=1), axis = 1)

        delta = self.reward + self.gamma * V_prime - V

        e_phi = T.sum(T.take(self.e, self.phi, axis=1), axis = 1)
        e_new = (self.lambda_ * self.gamma).dimshuffle(0,'x') * self.e
        e_new = T.inc_subtensor(e_new[:, self.phi], 1-(self.alpha * self.gamma * self.lambda_*e_phi).dimshuffle(0,'x'))

        theta_new = self.theta + (e_new * (self.alpha * (delta + V - self.V_old)).dimshuffle(0,'x'))
        theta_new = T.inc_subtensor(theta_new[:,self.phi], -(self.alpha * (V - self.V_old)).dimshuffle(0,'x'))
        ...
        self._predict = theano.function(
            [self.phi_prime],
            T.sum(self.theta[:, self.phi_prime], axis = 1)
            )
    ...
    def update(self, phi_prime, reward, alpha=None, lambda_=None, gamma=None):
        ...
        self._update(alpha.astype(theano.config.floatX).astype(theano.config.floatX), lambda_.astype(theano.config.floatX).astype(theano.config.floatX), gamma.astype(theano.config.floatX), phi_prime.astype(np.int32), np.atleast_1d(reward).astype(theano.config.floatX))

    def predict(self, phi):
        """
        Returns the current prediction for a given set of features phi.
        """
        return self._predict(phi.astype(np.int32))
